...
export const metadata: Metadata = buildMetadata({
  ...
  description: "Tajir Point POS for kiryana stores, apparel shops, and hardware stores in Pakistan, UAE, and Saudi Arabia. Khata ledger, barcode scanning, works without internet, and FBR/ZATCA compliance.",
  path: "/solutions/general-retail",
});

const FEATURES = [
  { icon: "/icons/offline.png",    title: "Works without internet",  body: "Sell even when the internet goes down. Every sale, return, and discount is saved locally and syncs the moment you're back online." },
  { icon: "/icons/inventory.png",  title: "Inventory management",    body: "Track stock by unit, carton, or kilogram. Get low-stock alerts before you run out. Set re-order levels per product." },
  { icon: "/icons/accounting.png", title: "Khata ledger",            body: "Record credit sales and track what every customer owes. Send WhatsApp reminders when payments are due." },
  { icon: "/icons/fbr.png",        title: "FBR compliance (PK)",     body: "Every sale transmits to FBR in real time. POS-IRN generation, e-invoicing, and Annex-C export — fully automatic." },
  { icon: "/icons/barcode.png",    title: "Barcode & label printing", body: "Scan barcodes at checkout or print your own shelf labels for any product. Supports USB, Bluetooth, and network scanners." },
  { icon: "/icons/report.png",     title: "Daily & monthly reports", body: "Z-report at shift end, daily sales summary, top-selling products, and profit margin per item." },
  { icon: "👥",                    title: "Multi-user access",       body: "Cashier, supervisor, and owner roles. Cashiers can't see cost prices or delete sales without manager approval." },
  { icon: "/icons/printer.png",    title: "Thermal receipt printing", body: "Print to any 58mm or 80mm Bluetooth or USB thermal printer. Customize your receipt header, footer, and logo." },
];
...
export default function GeneralRetailPage() {
  return <SolutionPage
    ...
    headline={<>The POS built for<br />the neighbourhood shop.</>}
...
function SolutionPage({
  vertical, subtitle, headline, description, accentColor, features, ctaLabel,
}: {
  vertical: string;
  subtitle: string;
    ...
    <main id="main-content" className="min-h-screen bg-[var(--color-bg)]">
      <section className="pt-20 pb-16 lg:pt-28 lg:pb-20">
        <div className="mx-auto max-w-[1320px] px-7 lg:px-10">
          ...
          <div className="inline-flex items-center gap-2 text-[13px] font-semibold mb-4 px-3 py-1.5 rounded-full border" style={{ borderColor: `${accentColor}40`, color: accentColor, background: `${accentColor}12` }}>
            {vertical}
          </div>
          ...
          <p className="mt-5 text-[17px] text-[var(--color-muted)] leading-[1.65] max-w-[520px]">{description}</p>
              ...
              <div key={f.title} className="rounded-[20px] border border-[var(--color-line)] bg-[var(--color-bg-2)] p-6">
                <div className="mb-3">
                  {f.icon.startsWith("/") ? (
                    ...
                    <img src={f.icon} alt="" width={52} height={52} className="w-[52px] h-[52px] object-contain" />
                  ) : (
      ...
      <section className="py-16 bg-[var(--color-ink)]">
        <div className="mx-auto max-w-[1320px] px-7 lg:px-10 text-center">
          ...
          <p className="mt-4 text-[16px] text-white/55 max-w-[400px] mx-auto">Set up in minutes. Your data, your language, your compliance — ready from day one.</p>
