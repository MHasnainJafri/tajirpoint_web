...
export const metadata: Metadata = buildMetadata({
  ...
  description: "Tajir Point for electronics and mobile phone shops in Pakistan, UAE, and Saudi Arabia. IMEI tracking, serial numbers, warranty management, repair job cards, and supplier ledger.",
  path: "/solutions/electronics",
});

const FEATURES = [
  { icon: "📱", title: "IMEI & serial tracking", body: "Every unit is tracked by IMEI or serial number from purchase to sale. Full chain of custody — no unit ever goes missing." },
  { icon: "🔖", title: "Warranty register", body: "Log warranty start/end per unit at time of sale. Customers can claim warranty by IMEI — you see the full purchase record instantly." },
  { icon: "🔧", title: "Repair job cards", body: "Create job cards for repairs, assign to a technician, track parts used, and notify customers when the device is ready." },
  { icon: "📦", title: "Box-level inventory", body: "Stock phones by model, color, and storage variant. See exactly how many Redmi Note 13 128GB in Black you have." },
  { icon: "🏪", title: "Supplier management", body: "Track purchases per supplier, log advance payments, and see outstanding payables across all vendors at a glance." },
  { icon: "🧾", title: "Detailed receipts", body: "Receipts show IMEI, model, color, and warranty expiry. Customers get a receipt they can actually use for claims." },
  { icon: "💰", title: "Trade-in & exchange", body: "Log trade-in devices with IMEI, deduct the trade-in value from the new sale, and track the second-hand unit separately." },
  { icon: "📊", title: "Model-wise reports", body: "See which models sell fastest, your average margin per brand, and slow-moving stock before it becomes a problem." },
];
    ...
    <main id="main-content" className="min-h-screen bg-[var(--color-bg)]">
      <section className="pt-20 pb-16 lg:pt-28 lg:pb-20">
        <div className="mx-auto max-w-[1320px] px-7 lg:px-10">
          ...
          <h1 className="text-[44px] lg:text-[68px] font-extrabold tracking-[-0.04em] leading-[1.04] text-[var(--color-ink)] max-w-[800px]">
            Every unit tracked,<br />from box to buyer.
          </h1>
          <p className="mt-3 text-[14px] text-[var(--color-muted)]">Mobile Phones · Accessories · Computers · Repair</p>
          <p className="mt-5 text-[17px] text-[var(--color-muted)] leading-[1.65] max-w-[520px]">
            IMEI tracking, warranty registers, repair job cards, and serial-level inventory. Built for shops where every unit has a unique identity.
          </p>
      ...
      <section className="py-16 bg-[var(--color-ink)]">
        <div className="mx-auto max-w-[1320px] px-7 lg:px-10 text-center">
          ...
          <p className="mt-4 text-[16px] text-white/55 max-w-[400px] mx-auto">We'll walk you through IMEI tracking and warranty setup in your demo — live, on your own data.</p>
