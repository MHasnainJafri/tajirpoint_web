      ...
      <section className="pt-20 pb-16 lg:pt-28 lg:pb-20 bg-[var(--color-ink)] text-white">
        <div className="mx-auto max-w-[1320px] px-7 lg:px-10">
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            ...
            <Link
              href="/data-privacy"
              className="inline-flex items-center gap-2 text-[14px] font-semibold text-white/50 hover:text-white transition-colors"
            >
              Data Privacy →
            </Link>
          </div>
        ...
        <div className="mx-auto max-w-[1320px] px-7 lg:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {CONTROLS.map((group) => (
              <div
                key={group.category}
                className="rounded-[20px] border border-[var(--color-line)] bg-[var(--color-bg-2)] overflow-hidden"
              >
                <div className="px-6 py-4 border-b border-[var(--color-line)] bg-white">
                  <h3 className="font-bold text-[15px] text-[var(--color-ink)]">
                    {group.category}
                  </h3>
                </div>
                <div className="divide-y divide-[var(--color-line)]">
                  {group.items.map((item) => (
                    ...
                    >
                      <span className="text-[13.5px] text-[var(--color-muted)] shrink-0">
                        {item.label}
                      </span>
                      <span className="text-[13.5px] font-semibold text-[var(--color-ink)] text-end">
                        {item.value}
                      </span>
                    </div>
                  ))}
        ...
        <div className="mx-auto max-w-[1320px] px-7 lg:px-10">
          ...
          <div className="flex flex-wrap gap-3">
            {CERTIFICATIONS.map((c) => (
              <div
                key={c.name}
                className="flex items-center gap-3 rounded-[12px] border border-[var(--color-line)] bg-white px-4 py-3"
              >
                <span className="w-5 h-5 rounded-full bg-[var(--color-mint)]/15 flex items-center justify-center text-[var(--color-mint)] text-[9px] font-bold shrink-0">
                  ✓
                </span>
                <div>
                ...
                >
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className="text-[11px] font-mono text-[var(--color-muted)] border border-[var(--color-line)] rounded-full px-2 py-0.5">
                        {step.time}
                      </span>
                    </div>
        ...
        <div className="mx-auto max-w-[1320px] px-7 lg:px-10">
          <div className="max-w-[600px]">
            ...
            <div className="flex flex-wrap gap-3">
              <Link
                href="/security"
                className="text-[13.5px] font-semibold text-[var(--color-muted)] hover:text-[var(--color-ink)] transition-colors"
              >
                Security overview →
              </Link>
