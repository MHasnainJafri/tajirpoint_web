...
export async function Footer() {
  ...
  return (
    <footer className="bg-[#080808] text-white/60">
      {/* Top band — brand + newsletter */}
      <div className="border-b border-white/[0.06]">
        <Container>
          <div className="py-14 flex flex-col lg:flex-row lg:items-center gap-10 lg:gap-0 justify-between">
            {/* Brand */}
            <div>
              <Lockup variant="on-dark" />
              <p className="mt-4 text-[14px] text-white/45 max-w-[300px] leading-[1.6]">
                {t("tagline")}
              </p>
              {/* Social icons */}
              <div className="mt-5 flex items-center gap-2">
                {SOCIAL.map((s) => (
                  <a
                    ...
                    aria-label={s.label}
                    className="w-8 h-8 rounded-lg bg-white/[0.06] hover:bg-white/[0.14] flex items-center justify-center text-white/50 hover:text-white transition-colors"
                  >
                    {s.icon}
                  </a>
                ))}
      ...
      {/* Links — Solutions + Company only */}
      <Container>
        <div className="py-12 grid grid-cols-2 gap-10 lg:gap-20 max-w-[480px]">
          {siteConfig.nav.footer.map((group) => (
            <div key={group.heading}>
              <h6 className="text-[10.5px] tracking-[0.14em] uppercase text-white/40 font-semibold mb-5">
                {group.heading}
              </h6>
